...
function Damagelog:DrawDamageTab(x, y)
	...
	local function askLogs()
		...
		self.Damagelog:AddLine("", "", "#Damagelog.Loading")
		self.loading = {}
		self.receiving = true
		net.Start("DL_AskDamagelog")
		net.WriteInt(self.SelectedRound, 20)
		net.SendToServer()
	end
	...
	self.RF:SetName("#Damagelog.RoundFilter")
	...
	self.Filters:SetText("#Damagelog.EditFilter")
	...
	self.PS_Label.Text = "#Damagelog.CurrentFilter"
	...
	self.PlayersCombo:AddChoice("#Damagelog.NoPlayers", NULL)
	...
	self.Highlight.DoClick = function(self)
		...
		if #Damagelog.Highlighted >= 3 then
			Derma_Message("#Damagelog.MorePlayers", "#Damagelog.Error", "OK")
		else
	...
	self.DamageInfoBox:SetName("#Damagelog.DmgInfo")
	self.DamageInfo = vgui.Create("DListView")
	self.DamageInfo:SetHeight(90)
	self.DamageInfo:AddColumn("#Damagelog.DmgInfo").DoClick = function() end
	...
	self.Roles:AddColumn("#Damagelog.Player")
	self.Roles:AddColumn("#Damagelog.Role")
	self.Roles:AddColumn("#Damagelog.Alive")
	...
	show_innocents:SetText("#Damagelog.ShowInnocent")
	...
	self.Damagelog:AddColumn("#Damagelog.Time"):SetFixedWidth(40)
	self.Damagelog:AddColumn("#Damagelog.Type"):SetFixedWidth(40)
	self.Damagelog.EventColumn = self.Damagelog:AddColumn("#Damagelog.Event")
	...
	if LastMapExists then
		self.Round:AddChoice("#Damagelog.LastRound", -1)
	...
	if PlayedRounds > 1 or (LocalPlayer():CanUseDamagelog() and PlayedRounds > 0) then
		...
		for i = i_count, PlayedRounds do
			if i == PlayedRounds and LocalPlayer():CanUseDamagelog() then
				self.Round:AddChoice("#Damagelog.Current #Damagelog.Round", i)
			else
				self.Round:AddChoice("#Damagelog.Round "..tostring(i), i)
			end
			LastChoise = LastChoise + 1
		end
	...
	elseif not LastMapExists then
		self.Round:AddChoice("#Damagelog.NoLogsAvailable")
		self.Round:ChooseOptionID(1)
	end
...
function Damagelog:ReceiveLogs(empty, tbl, last)
	...
	self.Damagelog:Clear()
	if empty then
		self.Damagelog:AddLine("", "", "#Damagelog.Nothinghere")
	else
...
net.Receive("DL_RefreshDamagelog", function()
	...
	if IsValid(Damagelog.Damagelog) then
		...
		if lines[1] and lines[1]:GetValue(3) == "#Damagelog.Nothinghere" then
