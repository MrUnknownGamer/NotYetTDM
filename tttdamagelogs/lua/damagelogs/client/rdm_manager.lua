...
local function BuildReportFrame(report)
	...
	if ReportFrame and ReportFrame:IsValid() then
		...
		ReportFrame:SetTitle("#Damagelog.BeenReported")
		...
		ReportFrame.AddReport = function(ReportFrame, report)
			...
			Info:SetText(report.victim_nick.. " #Damagelog.ReportedYou"..(current and (" #Damagelog.AfterRound "..(report.round or "?")) or " #Damagelog.PreviousMap "))
			...
			Button:SetText("#Damagelog.Send")
			Button.DoClick = function()
				local text = string.Trim(TextEntry:GetValue())
				local size = #text
				if size < 10 then
					Info:SetText("#Damagelog.MinCharacters");
					...
					Info:SetText("#Damagelog.ResponseSubmitted")
					...
					ReportFrame:Close()
					ReportFrame:Remove()
					Damagelog:Notify(DAMAGELOG_NOTIFY_INFO, "#Damagelog.ResponseSubmitted", 4, "")
				end
			end
...
function Damagelog:ReportWindow(tbl)
	...
	Frame:SetTitle("RDM Manager - #Damagelog.ReportPlayer")
	Frame:SetSize(w,h)
	Frame:Center()
	...
	InfoLabel:SetText("#Damagelog.FalseReports")
	...
	UserList.AddPlayer = function(pnl, pl, is_killer, killer_valid)  
		...
		ply.Paint = function(self, w, h)
			...
			AAText((is_killer and "(#Damagelog.Killer) " or "")..pl:Nick(), "RDM_Manager_Player", 35, 7, c, TEXT_ALIGN_LEFT)
		end
	...
	Label:SetText("#Damagelog.ExplainSituation")
	...
	Submit:SetText("#Damagelog.Submit")
	...
	Submit.Think = function(self)
		...
		if disable then
			...
			Submit:SetText("#Damagelog.NotEnoughCharacters")
		else
	...
	Tabs:AddSheet("#Damagelog.ReportPlayer", ReportPanel, "icon16/report_user.png")
	
	local Logs = vgui.Create("DListView")
	
	Logs:AddColumn("#Damagelog.Time"):SetFixedWidth(40);
	Logs:AddColumn("#Damagelog.Type"):SetFixedWidth(40);
	Logs:AddColumn("#Damagelog.Event")
	
	if tbl then
		self:SetListViewTable(Logs, tbl, false)
	end
	
	Tabs:AddSheet("#Damagelog.LogsBeforeDeath", Logs, "icon16/application_view_list.png")
...
net.Receive("DL_SendForgive", function()
	...
	answer:SetTitle("#Damagelog.MessageFrom "..nick.." #Damagelog.AboutYourReport")
	answer:Center()
	answer:MakePopup()
	...
	forgive:SetText("#Damagelog.Forgive")
	...
	nope:SetText("#Damagelog.KeepReport")
	nope:SetPos(205, 235)
	nope:SetSize(188, 30)
...
net.Receive("DL_Answering_global", function(_len)
	...
	if not ply:IsActive() then
		chat.AddText(Color(255,62,62), nick, color_white, " #Damagelog.IsAnswering")
	end
end)