...
if (LocalPlayer():IsSuperAdmin() or LocalPlayer():IsAdmin()) then -- Can be verified on the client because it's only a notification
...
function Damagelog:OpenMenu()
	...
	self.Menu.PaintOver = function(self, w, h)
		...
		if self.AboutPos >= 50 then
			...
			surface.DrawText("Rewritten by Min")
			-- surface.SetTextPos(_x + 5, _y  + 25)
	...
	if outdated then
		...
		info:SetText("#Damagelog.UpdateNotify");
	...
	self.About:SetText("? #Damagelog.TextAbout")
	self.About.DoClick = function()
		self.Menu:About()
		self.About:SetText(self.Menu.AboutState and "? #Damagelog.TextAbout" or "? #Damagelog.TextAbout")
	end
end
...
net.Receive("DL_InformSuperAdmins", function()
	...
	if nick then
		chat.AddText(Color(255,62,62), nick, color_white, " #Damagelog.AbuseNote")
	end
end)

net.Receive("DL_Ded", function()
	if GetConVar("Damagelog_RDMManagerEnabled"):GetBool() and GetConVar("ttt_dmglogs_rdmpopups"):GetBool() and net.ReadUInt(1,1) == 1 then
		...
		frame:SetTitle("#Damagelog.PopupNote")
		frame:ShowCloseButton(false)
		frame:Center()
	
		local reason = vgui.Create("DLabel", frame)
		reason:SetText("#Damagelog.KilledBy "..death_reason)
		...
		report:SetText("#Damagelog.OpenMenu")
		...
		close:SetText("#Damagelog.WasntRDM")
		...
		chat.AddText(Color(255,62,62), "[RDM Manager] ", Color(255,255, 255), "#Damagelog.OpenReportMenu ", Color(98,176,255), Damagelog.RDM_Manager_Command, Color(255, 255, 255), " #Damagelog.Command.")
